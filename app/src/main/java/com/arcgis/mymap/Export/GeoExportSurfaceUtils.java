...
public class GeoExportSurfaceUtils {
    ...
    public static void writesurfaceExcel(Context context, List<SurFace> list, String fileName,String exportpath) throws IOException, WriteException {
        ...
        for (int i=0;i<list.size();i++){
            ...
            Label name=new Label(0,i+1,surFace.getName());
            Label code=new Label(1,i+1,surFace.getCode());
            Label jingdu=new Label(2,i+1, StringUtils.join(surFace.getListla(),","));
            Label weidu=new Label(3,i+1,StringUtils.join(surFace.getListln(),","));
            Label leixing=new Label(4,i+1,surFace.getClassification());
            Label miaoshu=new Label(5,i+1,surFace.getDescription());

            sheet.addCell(name);
            sheet.addCell(code);
            sheet.addCell(jingdu);
            sheet.addCell(weidu);
            sheet.addCell(leixing);
